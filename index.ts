  ...
  routes: [
    {
      ...
      eventHandler: new aws.lambda.CallbackFunction('saga', {
        ...
        callback: async ({ body: rawBody, headers }) => {
          ...
          // Immediately return a 200 response to Slack to prevent timeout
          // Process the GitHub workflow asynchronously
          const processWorkflowAsync = async () => {
          ...
          // Start the async process without waiting for it
          processWorkflowAsync().catch(err => console.error('Async workflow processing error:', err));

          // Return immediately with a 200 status code
          return { 
            statusCode: 200, 
